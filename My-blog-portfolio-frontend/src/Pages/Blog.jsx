...
const Blog = () => {
   ...
   const [Likes,setLikes]=useState(0)
        ...
        auth.onAuthStateChanged((user) => {
            ...
            if (user) {
                ...
                if (user.uid == admin) {
                    ...
                    setadminAcc(false)
                    axios.post("https://my-portfolio-blog-app.onrender.com/sentBlogs", { title: blogTitle, content: blogContent }).then(() => {
    ...
    const recentBlogs = () => {
        auth.onAuthStateChanged((user) => {
            if (user) {
                axios.get("https://my-portfolio-blog-app.onrender.com/api/blogs/getBlogs").then((getBlogdata) => {
                    ...
                    BlogsArr.map(function (item, index) {
                        return <div key={index} className=" bg-gray-300 w-[95%] p-4  rounded flex flex-col gap-3">
                            ...
                            <div className="flex gap-2 items-center">
                                <button  style={{ boxShadow: "rgba(0, 0, 0, 0.35) 0px 5px 15px" }} className="bg-[#FDC010] py-1 px-2 rounded-md ">Likes :</button><span className="flex text-xl items-center">{Likes} <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6">
